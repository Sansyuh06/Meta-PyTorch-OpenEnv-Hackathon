...
class ActionHandler:
    ...
    def __init__(self) -> None:
        ...
        for cve_id in _KNOWN_CVE_IDS:
            ...
            try:
                with open(fixture_path, encoding="utf-8") as fh:
                    ...
                    required_keys = {
                        "nvd_data", "advisory_data", "gav_data",
                        "method_data", "patch_diff",
                        "synthetic_code_snippet", "ground_truth",
                    }
                    missing = required_keys - set(data.keys())
                    if missing:
                        raise ValueError(
                            f"Fixture {cve_id} missing keys: {missing}"
                        )
        ...
        # Dispatch table — dict map, not if/elif chains
