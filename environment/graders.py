...
class Grader:
    ...
    def _grade_medium(
        ...
        if sub_method == gt_method:
            ...
            msgs.append(
                f"Vulnerable method incorrect "
                f"(got '{sub_method}', expected '{gt_method}')"
            )
        ...
        penalty = max(-0.3, -0.1 * redundant)  # cap penalty at -0.3
        bd["redundancy_penalty"] = penalty
        if penalty < 0:
            msgs.append(
                f"Redundancy penalty ({redundant} extra repeats): {penalty:.2f}"
            )
    ...
    def _grade_hard(
        ...
        if sub_invoked == gt_invoked:
            ...
            msgs.append(
                f"Invocation detection wrong "
                f"(got {sub_invoked}, expected {gt_invoked})"
            )
        ...
        penalty = max(-0.3, -0.05 * oversteps)  # cap penalty at -0.3
        bd["overstep_penalty"] = penalty
        if penalty < 0:
            msgs.append(
                f"Over-step penalty ({oversteps} steps over limit): {penalty:.2f}"
            )
